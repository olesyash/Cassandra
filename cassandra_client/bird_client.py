...
class BirdClient:
    ...
    def parse_and_log_trace(self, operation_type, bird_id, trace, operation_details):
        ...
        try:
            with open(self.trace_log_file, "a", encoding="utf-8") as f:
                ...
                f.write(
                    f"Total Duration: {trace.duration.total_seconds() * 1000000:.0f} microseconds\n"
                )
                ...
                if hasattr(trace, "events") and trace.events:
                    for i, event in enumerate(trace.events, 1):
                        ...
                        if hasattr(source_elapsed_raw, "total_seconds"):
                            source_elapsed = int(
                                source_elapsed_raw.total_seconds() * 1000000
                            )
                        else:
                            source_elapsed = (
                                int(source_elapsed_raw) if source_elapsed_raw else 0
                            )
            ...
            try:
                ...
                if bird_id == self.traced_bird_id:
                    ...
                    result = self.session.execute(
                        insert_stmt,
                        (bird_id, date, timestamp, species, latitude, longitude),
                        trace=True,
                    )
                    ...
                    operation_details = (
                        f"INITIAL INSERT location ({latitude:.4f}, {longitude:.4f})"
                    )
                    self.parse_and_log_trace(
                        "INSERT", bird_id, trace, operation_details
                    )

                    print(
                        f"  ✓ TRACED INITIAL INSERT for {bird_id} ({species}) at ({latitude:.4f}, {longitude:.4f}) - Trace logged"
                    )
                else:
            ...
            for i in range(self.num_birds):
                ...
                try:
                    ...
                    if bird_id == self.traced_bird_id:
                        ...
                        self.parse_and_log_trace(
                            "UPDATE", bird_id, trace, operation_details
                        )

                        print(
                            f"  ✓ TRACED UPDATE {bird_id} location to ({latitude:.4f}, {longitude:.4f}) - Trace logged"
                        )
                    else:
    ...
    def run(self):
        ...
        try:
            ...
            print(f"Update interval: {self.update_interval} seconds")
            print(
                f"🔍 TRACING ENABLED for {self.traced_bird_id} - UPDATE operations will be traced"
            )
            ...
            print(
                f"   - Total operations traced: {1 + self.updates_per_bird} (1 INSERT + {self.updates_per_bird} UPDATEs)"
            )
            print(f"   - Trace log file: {self.trace_log_file}")
            print(
                f"   - Check the trace file for detailed coordinator/replica flow analysis"
            )
